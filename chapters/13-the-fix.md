...
![A two-column diagnostic infographic. The left column labeled "Content problem" is stamped SOLVED and lists HarvardX DPI 681M, CS50 AI and Prompt Engineering, MIT OpenCourseWare, Stanford Online, and Carnegie Mellon. The right column labeled "Incentive problem" is stamped UNTOUCHED and lists salary schedule, licensure hours, evaluation rubric, portable credential, and stipend or release time. A flow arrow under both columns shows district AI-training dollars flowing almost entirely into the left column, with negligible flow to recognition.](../images/13-the-fix-fig-01.png)
*Figure 13.1 — Content problem vs incentive problem. One is solved. The other has not been touched.*
...
![A horizontal bar chart of three per-student annual spending patterns on a shared zero baseline. ASSISTments costs $46 per student per year, almost entirely teacher time and coaching, with statistically significant math gains. Prevailing district EdTech procurement runs $1,000 per student per year with roughly two-thirds of licenses never opened, shown as a hatched portion of the bar. A proposed reallocation spends $150 on a HarvardX verified certificate and reserves $850 per teacher for peer time, study groups, and stipends.](../images/13-the-fix-fig-02.png)
*Figure 13.2 — Per-student annual spending. Three patterns; one zero baseline; a twenty-two-fold spending gap and no comparable outcome evidence.*
...
![A side-by-side comparison table of compliance layers across four dimensions. Medicine column lists ACCME as accreditor, roughly 200 CME credits per five-year cycle, and quarterly knowledge assessments tied to clinical reasoning. Teaching column lists state PD frameworks with no national equivalent, 20 to 50 PD clock-hours per year, and a sign-in sheet for assessment. The final row, highlighted in red as the fault line, contrasts the audit question at the end of the activity: medicine asks whether the physician completed a verified learning activity, while teaching asks whether the teacher was in the room.](../images/13-the-fix-fig-03.png)
*Figure 13.3 — Medicine vs teaching compliance layers. A system that measures attendance will get attendance; the fault line is the audit question.*
...
![A mockup of a one-page superintendent's memo on the Office of the Superintendent letterhead. Three numbered moves are stacked on the page. Move one issues an administrative letter recognizing HarvardX, MITx, Stanford Online, and Carnegie Mellon completion at a published credit ratio (signal: learning is valued). Move two authorizes the study-group documentation pathway using the existing PLC structure with a one-page monthly template (signal: trying is valued). Move three, accented in red, adds an AI-fluency salary-increment tier at the next contract window with peer-review borrowed from National Board Certification (signal: practice change is valued). Each move is annotated "new budget required: $0." A signature line at the bottom reads "— signed, the superintendent."](../images/13-the-fix-fig-04.png)
*Figure 13.4 — The one-page superintendent's memo. Three administrative decisions; one signature; zero new budget.*
