...
![Grouped bar chart of the Bastani 2025 PNAS results. Three conditions: No-AI control (baseline at 0 on both measures), GPT-Base (practice-session correct-answer rate +48% above control, but closed-book exam score 17% below control), and GPT-Tutor (practice gain about +30%, exam outcome approximately 0 — no decrement). The chart highlights how unguided AI inflates in-session performance while harming exam retention, and how scaffolded AI neutralizes the harm.](../images/07-the-cognitive-offloading-problem-fig-01.png)
*Figure 7.01 — Bastani 2025 PNAS: practice gains vs. exam outcomes by condition*
...
![Flow diagram of the durable-learning chain: prediction error (student commits a guess; the wrongness is the update; Schultz et al. 1997) leads to effortful retrieval (NMDA-channel calcium influx, PKA cascade, structural synaptic growth; Roediger & Karpicke 2006), leads to schema construction (pattern migrates from working to long-term memory; Sweller 1988; Cowan 2001), leads to durable memory that retrieves under exam. A parallel bypass arrow labeled "unguided AI offloading" routes from "tool supplies the answer before the student commits" around all three nodes, landing at "fluency without storage strength" (Bjork & Bjork 1992).](../images/07-the-cognitive-offloading-problem-fig-02.png)
*Figure 7.02 — The neurobiology chain that unguided AI bypasses*
...
![Two-column comparison labeled "Feels like understanding" versus "Is understanding." The left column lists smooth, fluent, confident cues — re-reading the page, watching the clear YouTube explanation, AI hands the answer — culminating in the metacognitive cue "I know this." The right column lists effortful, frictional, pessimistic cues — student commits and fails, retrieval practice, interleaved problem types, spaced review — culminating in "I can actually do this." A double-headed arrow between the columns labels the Bjorks' inverse coupling: when retrieval is easy in the moment, storage strength does not build; when retrieval is effortful and successful, it does.](../images/07-the-cognitive-offloading-problem-fig-03.png)
*Figure 7.03 — The fluency trap: what learning feels like is not what learning is*
...
| Source | Concept introduced | What it establishes for the AI-offloading argument |
|---|---|---|
| Sweller (1988), *Cognitive Science* | Cognitive load; working-memory bottleneck | Learning is the migration of patterns from working memory into long-term schemas. Skip the migration and nothing durable is built. |
| Bjork & Bjork (1992) | Storage strength vs. retrieval strength | When retrieval is easy, retrieval events build almost no storage strength. AI maximizes retrieval ease and minimizes storage gain. |
| Schultz, Dayan, & Montague (1997), *Science* | Dopamine encodes reward prediction error, not reward | Brains update synapses in proportion to surprise. A student who never commits an answer never generates the error signal. |
| Cowan (2001), *BBS* | Working memory holds ~4 chunks | The narrow bottleneck that schemas exist to bypass. Without schemas, every problem is reconstructed from scratch. |
| Koriat & Bjork (2005), *J. Exp. Psych.* | Illusions of competence in self-monitoring | Fluent learning yields confident predictions and poor recall; effortful learning yields pessimistic predictions and strong recall. |
| Roediger & Karpicke (2006), *Psych. Science* | Testing effect | Successful effortful retrieval drives consolidation; re-reading does not. ChatGPT is a fluent retrieval substitute. |
| Risko & Gilbert (2016), *Trends Cog. Sci.* | Cognitive offloading | Offloading is not learning-destructive in itself — but offloading the operation a lesson is designed to teach prevents the learning event. |
