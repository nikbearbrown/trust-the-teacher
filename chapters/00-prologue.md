...
![Grouped bar chart contrasting student AI use at 92, 94, and 62 percent with teacher daily use at 12–15 and 7 percent on a shared zero-baseline scale.](../images/00-prologue-fig-01.png)
...
![Two parallel timelines across eighteen months: roughly fourteen AI capability releases at a four- to six-week cadence above, and a single PD module passing through funded, designed, scheduled, and delivered stages below, with arrows showing the delivered module already overtaken twice.](../images/00-prologue-fig-02.png)
...
![Horizontal segmented bar showing four tiers of K-12 educator AI training access: 44 percent none, 29 percent one workshop, 19 percent intermediate, 8 percent systematic — the trained tier the smallest sliver on the right.](../images/00-prologue-fig-03.png)
...
Build a single-pane grouped bar chart on a shared zero-baseline percentage axis (0–100%). Render six bars in two visually divided groups, left to right: Students — University regular use 92%, Assessed coursework 94%, US middle/high school 62% — and Teachers — K-12 daily use 12–15% (plot midpoint 13.5%, label "12–15%"), Business-school faculty daily 7%, B-school faculty occasional 67% (render as a dashed-outline reference bar, not a filled bar). Chart type: vertical bar chart with categorical x and quantitative y. Color: student bars in `var(--color-red)`, teacher bars in `var(--color-secondary)`, the reference bar as stroke-only with `stroke-dasharray`. Place group headers ("Students" / "Teachers") above the bars in EB Garamond. Draw a thin `var(--color-border)` divider between the two groups. Light dashed gridlines at 25/50/75. Bar values labeled above each bar in Inter 12px bold. Tooltips on hover and on keyboard focus give the source and population. Standalone HTML, D3 7.9.0 from the pinned CDN, accessible markup (role="img", title, desc, tabindex, aria-label per bar), ResizeObserver redraw.

> Reference implementation: `d3/00-prologue-fig-01.html`

---

### Figure 0.2 — Two clocks running at incompatible speeds

Build a two-lane parallel-timeline figure across an 18-month linear x-axis. Top lane: AI capability releases, plotted as 14 evenly spaced dots (~one every 1.3 months) along a solid `var(--color-ink)` track; label release v1, v4, v7, v10, v13, v14 above the track in JetBrains Mono. Bottom lane: a single institutional PD module passing through four stages at month 0 (funded), month 3 (designed), month 8 (scheduled), and month 15 (delivered) along a `var(--color-secondary)` track; draw faint segments connecting consecutive stages, and render "delivered" as a larger ring outline to mark arrival. Highlight a vertical ochre band (`var(--color-ochre)` at 10% opacity) covering month 13.5–16.5 labeled "delivery window." Draw two dashed annotation lines from the delivered point up to the two most recent capability releases inside that window, labeled "overtaken twice." Month-axis ticks at 0/6/12/18. Tooltips on every dot. Standalone HTML, D3 v7, inline CSS/JS, accessible, ResizeObserver.

> Reference implementation: `d3/00-prologue-fig-02.html`

---

### Figure 0.3 — How thin the "trained" tier actually is

Build a single horizontal segmented bar (one row, four segments) representing 100% of K-12 educators across four tiers of AI professional-development access: No PD 44%, One workshop 29%, Intermediate 19%, Systematic 8%. Chart type: stacked single-bar with a linear 0–100 x-scale. Color sequence left to right: `var(--color-ink)` (severe), `var(--color-secondary)` (limited), `var(--color-border)` (partial), `var(--color-ochre)` (trained — the one warm accent, used here because the "trained" sliver is the chapter's punchline). In each segment, render the percentage centered in Inter 14px bold (white text on the two dark segments, ink on the two light). Above each segment, render two stacked labels — tier name in EB Garamond 13px and a sub-line in Inter 11px `var(--color-secondary)`. Below the bar, render cumulative tick marks at 0/44/73/92/100 with `var(--color-secondary)` tick labels. Add two annotation callouts under the bar: one pointing at the 44% segment ("Nearly half — have received zero AI training of any kind"), one pointing at the 8% segment ("8% of the profession — has the training the moment requires"). Tooltips on each segment. Standalone HTML, D3 v7, accessible, ResizeObserver.

> Reference implementation: `d3/00-prologue-fig-03.html`
